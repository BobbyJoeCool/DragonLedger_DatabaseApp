...
describe('transformClass', () => {
  it("parses Cleric's CORE_TRAITS_TABLE for proficiencies and a single primary ability", () => {
    ...
    const { row } = transformClass(raw, 'test-source', '5.5e')
  ...
  it('explodes grouped gained_at levels into one ContentClassFeature entry per level', () => {
    const raw = loadFixtureResult<Open5eClass>('cleric.json')
    const { features } = transformClass(raw, 'test-source', '5.5e')
